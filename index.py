...
# ==============================================================================
# FILE CONFIGURATIONS & CLEAR COMMAND
# ==============================================================================
ENV_FILE = ".env"
...
def load_env_or_prompt():
    ...
    if missing_or_empty:
        print("\n" + "="*60)
        print("⚙️  IPGUARDEX V2.0.0 - INITIAL SETUP ⚙️")
        print("="*60)
        ...
        if 'DOWN_DELAY' in missing_or_empty:
            delay_input = input("🔹 Enter Delay Before First Down Alert (in minutes, Default 0): ").strip() or "0"
...
# ==============================================================================
# TIME FORMATTING UTILITY (For Telegram & Logs)
# ==============================================================================
def format_pretty_time(seconds):
    """Converts seconds into a beautiful readable format (e.g., 2 ঘণ্টা 15 মিনিট 30 সেকেন্ড)"""
    seconds = int(seconds)
    h = seconds // 3600
    m = (seconds % 3600) // 60
    s = seconds % 60
    ...
    time_parts = []
    if h > 0: time_parts.append(f"{h} ঘণ্টা")
    if m > 0: time_parts.append(f"{m} মিনিট")
    if s > 0 or not time_parts: time_parts.append(f"{s} সেকেন্ড")
...
def bg_monitoring_loop():
    ...
    while True:
        try:
            ...
            time_string = datetime.now().strftime("%I:%M:%S %p - %d/%m/%Y")
            ...
            for name, target in TARGETS.items():
                ...
                if not is_online:
                    ...
                    if name not in LIVE_DOWNTIMES:
                        ...
                        pretty_time = format_pretty_time(down_duration)
                        ...
                        if not down_data.get('first_alert_sent', False) and down_duration >= DOWN_DELAY:
                            send_telegram_message(
                                ...
                                f"⏰ *Time:* {time_string}\n"
                                f"⏳ *Down For:* {pretty_time}\n"
                                f"⚠️ *Status:* CRITICAL / OFFLINE"
                            )
                            ...
                            if current_time - last_alert >= DOWN_ALERT_INTERVAL:
                                send_telegram_message(
                                    f"⚠️ *REMINDER: SERVER STILL DOWN!*\n\n"
                                    f"🖥️ *Host:* {name}\n"
                                    f"🌐 *Target:* {target_label}\n"
                                    f"⏳ *Total Downtime:* {pretty_time}"
                                )
                                LIVE_DOWNTIMES[name]['last_alert_sent'] = current_time
                                save_state()
                    ...
                    if name in LIVE_DOWNTIMES:
                        ...
                        pretty_time = format_pretty_time(total_down_time)
                        ...
                        if LIVE_DOWNTIMES[name].get('first_alert_sent', False):
                            send_telegram_message(
                                ...
                                f"🌐 *Target:* {target_label}\n"
                                f"🎉 *Status:* ONLINE\n"
                                f"⏳ *Downtime Duration:* {pretty_time} পর অনলাইন হয়েছে"
                            )
                        ...
                        HISTORY_LOGS.append({
                            ...
                            'details': f"Down for {pretty_time}"
                        })
                        if len(HISTORY_LOGS) > 200: HISTORY_LOGS.pop(0)
...
# ==============================================================================
# FLASK WEB SERVER API & AURORA INTERFACE (V2.0.0 - Tabbed UI)
# ==============================================================================
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>IPGuardEx V2.0 - {{ web_name }}</title>
    ...
    <style>
        ...
        .server-card { background: linear-gradient(180deg, rgba(30, 41, 59, 0.4) 0%, rgba(15, 23, 42, 0.4) 100%); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 12px; transition: all 0.3s; backdrop-filter: blur(10px); }
        .server-card:hover { transform: translateY(-4px); border-color: rgba(139, 92, 246, 0.4); box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.5); }
        ...
        .tab-btn.active { border-color: #818cf8; color: #c7d2fe; }
...
<body class="p-3 md:p-6 lg:p-8">
    <div class="max-w-[1600px] mx-auto space-y-6">
        ...
        <div class="glass-panel p-4 flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-xl font-bold tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-indigo-400 to-purple-400 uppercase">IPGuardEx • {{ web_name }}</h1>
                    <p class="text-[10px] text-slate-500 font-mono">v2.0.0 • Developed by johirxofficial</p>
                </div>
            </div>
            <div class="flex gap-6 text-sm font-semibold">
                <button onclick="switchTab('dashboard')" id="tab-dashboard" class="tab-btn active text-slate-400 hover:text-indigo-300 pb-1"><i class="fa-solid fa-border-all mr-1"></i> Dashboard</button>
                <button onclick="switchTab('logs')" id="tab-logs" class="tab-btn text-slate-400 hover:text-indigo-300 pb-1"><i class="fa-solid fa-clock-rotate-left mr-1"></i> Logs</button>
                <button onclick="switchTab('settings')" id="tab-settings" class="tab-btn text-slate-400 hover:text-indigo-300 pb-1"><i class="fa-solid fa-gear mr-1"></i> Settings</button>
            </div>
        </div>
        ...
        <div id="content-dashboard" class="tab-content active space-y-6">
            ...
            <div class="glass-panel p-5">
                <div class="flex justify-between items-center mb-5">
                    <h2 class="text-sm font-bold flex items-center gap-2 text-slate-300 uppercase tracking-widest"><i class="fa-solid fa-network-wired text-indigo-400"></i> Active Nodes</h2>
                    <button onclick="openModal()" class="px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 rounded-lg font-bold text-xs transition-all shadow-lg text-white flex items-center gap-2"><i class="fa-solid fa-plus"></i> ADD</button>
                </div>
                <div id="hosts-grid" class="grid grid-cols-2 min-[480px]:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-3"></div>
            </div>
        </div>

        <!-- Logs Tab -->
        <div id="content-logs" class="tab-content space-y-6">
            <div class="glass-panel p-5">
                <h2 class="text-sm font-bold mb-4 flex items-center gap-2 text-slate-300 uppercase tracking-widest"><i class="fa-solid fa-list-check text-cyan-400"></i> Up/Down History</h2>
                <div id="history-box" class="max-h-[500px] overflow-y-auto space-y-2 pr-2"></div>
            </div>
        </div>

        <!-- Settings Tab -->
        <div id="content-settings" class="tab-content space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="glass-panel p-5 space-y-4">
                    <h2 class="text-sm font-bold flex items-center gap-2 text-slate-300 uppercase tracking-widest"><i class="fa-solid fa-bell text-amber-400"></i> Alert Configuration</h2>
                    <div class="bg-slate-900/60 p-3 rounded-lg border border-slate-700/50 text-xs font-mono text-slate-300 space-y-2">
                        <p><span class="text-slate-500">Bot Token:</span> ********<span id="set-token-end"></span></p>
                        <p><span class="text-slate-500">Chat ID:</span> <span id="set-chatid"></span></p>
                        <p><span class="text-slate-500">Alert Delay:</span> <span id="set-delay"></span> minutes</p>
                        <p><span class="text-slate-500">Reminder Interval:</span> <span id="set-interval"></span> minutes</p>
                    </div>
                    <p class="text-[10px] text-red-400/70"><i class="fa-solid fa-info-circle"></i> To change settings, run <code class="bg-slate-800 px-1 rounded">python3 index.py --clear</code> in terminal & restart daemon.</p>
                </div>
                
                <div class="glass-panel p-5 space-y-4">
                    <h2 class="text-sm font-bold flex items-center gap-2 text-slate-300 uppercase tracking-widest"><i class="fa-solid fa-database text-emerald-400"></i> Data Management</h2>
                    <div class="space-y-3">
                        <button onclick="exportData()" class="w-full py-2.5 bg-emerald-600/20 border border-emerald-500/30 hover:bg-emerald-600/40 text-emerald-400 rounded-lg text-xs font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-2"><i class="fa-solid fa-file-export"></i> Export Targets (JSON)</button>
                        
                        <div class="relative w-full">
                            <input type="file" id="importFile" accept=".json" onchange="importData(event)" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer">
                            <div class="w-full py-2.5 bg-cyan-600/20 border border-cyan-500/30 hover:bg-cyan-600/40 text-cyan-400 rounded-lg text-xs font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-2 pointer-events-none"><i class="fa-solid fa-file-import"></i> Import Targets (JSON)</div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="glass-panel p-5 border border-red-500/20">
                <h2 class="text-sm font-bold flex items-center gap-2 text-red-400 uppercase tracking-widest mb-4"><i class="fa-solid fa-skull-crossbones"></i> Danger Zone</h2>
                <p class="text-xs text-slate-400 mb-4">Factory reset will delete all targets, logs, and configuration files.</p>
                <button onclick="factoryResetUI()" class="px-4 py-2 bg-red-600/20 border border-red-500/30 hover:bg-red-600/40 text-red-400 rounded-lg text-xs font-bold uppercase tracking-wider transition-all"><i class="fa-solid fa-trash mr-1"></i> Factory Reset via Terminal</button>
            </div>
        </div>
    ...
    <script>
        ...
        function exportData() { window.location.href = '/api/export_targets'; }

        function importData(event) {
            const file = event.target.files[0]; if(!file) return;
            const reader = new FileReader(); reader.onload = function(e) { 
                if(confirm('This will overwrite existing targets. Are you sure?')) {
                    fetch('/api/import_targets', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: e.target.result })
                    .then(res => res.json()).then(res => { if(res.status === 'success') { loadData(); alert('Import successful!'); } else { alert('Import failed!'); } });
                }
            }; reader.readAsText(file);
        }

        function factoryResetUI() { alert("To factory reset, go to your terminal and run:\\nsudo systemctl stop ipguardex\\npython3 index.py --clear\\nsudo systemctl start ipguardex"); }
        ...
        function loadData() {
            fetch('/api/live_data').then(res => res.json()).then(data => {
                ...
                data.history.forEach(log => {
                    const statusColor = log.status === 'UP' ? 'emerald' : 'red';
                    historyBox.innerHTML += `<div class="px-4 py-2 bg-slate-900/40 rounded-lg border border-${statusColor}-500/10 flex justify-between items-center text-xs"><span class="font-bold text-slate-300">${log.name} <span class="text-[10px] px-1 bg-${statusColor}-500/10 text-${statusColor}-400 border border-${statusColor}-500/20 rounded ml-1">${log.status}</span></span><div class="flex gap-3 text-slate-400 font-mono text-[11px]"><span>${log.details}</span><span class="text-slate-500">${log.timestamp}</span></div></div>`;
                });

                // Update Settings Page
                document.getElementById('set-chatid').innerText = data.config.CHAT_ID;
                document.getElementById('set-delay').innerText = data.config.DOWN_DELAY;
                document.getElementById('set-interval').innerText = data.config.DOWN_ALERT_INTERVAL;
                if(data.config.BOT_TOKEN.length > 4) document.getElementById('set-token-end').innerText = data.config.BOT_TOKEN.slice(-4);
            });
        }
...
def get_live_data():
    return jsonify({
        ...
        "config": {
            "BOT_TOKEN": BOT_TOKEN,
            "CHAT_ID": CHAT_ID,
            "DOWN_DELAY": DOWN_DELAY // 60,
            "DOWN_ALERT_INTERVAL": DOWN_ALERT_INTERVAL // 60
        }
    })
...
# ==============================================================================
@app.route('/api/export_targets', methods=['GET'])
def export_targets():
    try:
        targets = load_targets()
        buffer = BytesIO()
        buffer.write(json.dumps(targets, indent=4).encode('utf-8'))
        buffer.seek(0)
        return send_file(buffer, as_attachment=True, download_name='IPGuardEx_Backup.json', mimetype='application/json')
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500

@app.route('/api/import_targets', methods=['POST'])
def import_targets():
    try:
        data = request.json
...
if __name__ == "__main__":
    ...
    print(f"🛡️ IPGuardEx V2.0.0 - Developed by johirxofficial")
    print(f"🚀 {WEB_NAME} Engine Started...")
    print(f"⏱️ Alert Delay: {DOWN_DELAY // 60} Minutes | Reminder Interval: {DOWN_ALERT_INTERVAL // 60} Minutes")
